...
\begin{document}
  ...
  \newcommand{\linearequationc}[2]{
    \text{\texttt{
        LinearEquation[sigma$= {#1}$, iota$= {#2}$]
    }}
  }


  % linear equation result
  \newcommand{\linearequationr}[2]{
    y = {#1}x + {#2}
  }


  \newcommand{\linearequation}[2]{
    \linearequationc{#1}{#2}\to \linearequationr{#1}{#2}
  }
  ...
  \newcommand{\getcount}{
    \arabic{chapter}.\arabic{scount}.\arabic{tcount}
  }
  ...
  % primary counter
  \newcounter{pcount}
  % setup secondary counter
  \newcounter{scount}[chapter]
  % \makeatletter
  % \@addtoreset{scount}{pcount}
  % \makeatother
  % set up tertiary counter
  \newcounter{tcount}
  \makeatletter
  ...
  {\remark Equivalence \stepcounter{tcount}\\
    In elementary algebra, one of the objects of our interest are equations. In general equations are of the form

    $$\text{\texttt{expession1}} = \text{\texttt{expression2}}$$

    When given such an equation, it is helpful to interpret it as a judgement of truth, as in "it is true that expression1 equals expression2". From this truthful equation, our goal is to infer further truthful equations, or truthful facts about this equation.\\
  }
  ...
  {\axiom Well Defined Operation \stepcounter{tcount}\\
  ...
  {\remark Isolating a Variable in an Equation \stepcounter{tcount}\\
  ...
  {\theorem Slope of a Linear Equation \stepcounter{tcount}\\
  ...
  {\remark Intercepts \stepcounter{tcount}\\
  ...
  {\theorem x, and y Intercepts of a Linear Equation \stepcounter{tcount}\\
  ...
  {\theorem Intercept From Two Points \stepcounter{tcount}

  }

  {\theorem Equivalent Definition of Slope (optional)  \stepcounter{tcount}
  ...
  {\remark Equation From 2 Points (Derivation) \stepcounter{tcount}
